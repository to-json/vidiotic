...
pub fn spawn(path: PathBuf) -> anyhow::Result<DecodeHandle> {
    ...
    let join = std::thread::spawn(move || {
        if let Err(e) = run(&path, &frame_tx, &close_rx) {
...
fn run(path: &PathBuf, tx: &Sender<DecodedFrame>, close_rx: &Receiver<()>) -> anyhow::Result<()> {
    ...
    if is_hap {
        ...
        run_hap(&mut ictx, tx, close_rx, vid_idx, tb, width, height, texture_count)
    } else {
        log::info!(
        ...
        run_software(&mut ictx, tx, close_rx, vid_idx, params, tb)
    }
}
...
fn run_software(
    ...
    loop {
        ...
        // Flush the decoder's buffered frames, then loop back to the start.
        decoder.send_eof()?;
        while decoder.receive_frame(&mut decoded).is_ok() {
            let mut rgba = ff::frame::Video::empty();
            scaler.run(&decoded, &mut rgba)?;
